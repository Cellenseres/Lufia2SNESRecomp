    ...
    if (Random32() & 1u) {
            ...
            g_bus.wram[0x099cu] &= 0xfeu;
        {
            ...
            if (!(Random32() & 3u)) {
                ...
                const uint8_t id = kBounds[Random32() & 3u];

                g_bus.wram[(uint16_t)(text + 1u)] = id;
                g_bus.wram[0x05fau + Random32() % 0x28u] = id;
                g_bus.wram[0x105fau + Random32() % 0x28u] = id;
            }
...
static void SeedF9E9(CpuState *cpu) {
    SeedJslX16(cpu, 0x81);
    g_bus.wram[0x09fau] = (uint8_t)(Random32() & 7u);
    g_bus.wram[0x09feu] = (uint8_t)(1u + Random32() % 99u);
    g_bus.wram[0x09ffu] = 0;
}
